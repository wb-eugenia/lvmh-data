...
with open('test_dup.csv', 'rb') as f:
    ...
    resp = requests.post('http://127.0.0.1:8082/api/data-cleaning', files=files)
    data = resp.json()
    print('=' * 50)
    ...
    print(f"Doublons: {data['report']['duplicates_removed']}")
    print(f"Vides: {data['report']['empty_rows_removed']}")
