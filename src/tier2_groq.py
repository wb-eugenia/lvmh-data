...
class Tier2Groq:
    """
    Tier 2 implementation using Groq API (Optimized).
    Features:
    - Enhanced System Prompt (Taxonomy, Rules, Few-shot)
    - Smart Model Selection (Mixtral-8x7b Balanced)
    ...
    MODELS = {
        'fast': 'llama-3.1-8b-instant',       # Speed priority
        'balanced': 'llama-3.3-70b-versatile', # Best balance (Mixtral deprecated)
        'quality': 'llama-3.3-70b-versatile', # Quality priority
    }
...
"""
    ...
    def __init__(self, model_tier: str = 'balanced'):
        ...
        logger.info(f"Tier 2 Groq initialized with model: {self.model}")
    ...
    def _check_circuit_breaker(self) -> bool:
        ...
        logger.warning("Circuit breaker OPEN - too many Groq failures")
        return False
    ...
    def _record_success(self):
        ...
        if self.circuit_breaker['failures'] > 0:
            logger.info(f"Groq success after {self.circuit_breaker['failures']} failures - resetting")
            self.circuit_breaker['failures'] = 0
    ...
    @safe_execution(default_return=ExtractionResult(extracted_by="tier2_groq_fallback", processing_tier="tier2", confidence=0.0))
    @retry_with_backoff(retries=2)
    async def extract(self, text: str, language: str = 'FR') -> ExtractionResult:
        """
        Async extraction with Timeout, Circuit Breaker, and Metrics.
        """
        start_time = time.time()
        
        # Check circuit breaker
        if not self._check_circuit_breaker():
            raise Exception("Circuit breaker OPEN - Groq unavailable")
        ...
        try:
            ...
            raise Exception(f"Groq timeout ({self.timeout_seconds}s)")
            
        except json.JSONDecodeError as e:
            logger.error(f"Groq JSON parsing error: {e}")
            self._record_failure()
            self.metrics['total_json_errors'] += 1
            self._record_extraction(None, 0, success=False)
            raise Exception(f"Invalid JSON from Groq: {e}")
    ...
    def _format_result_safe(self, result_dict: Dict) -> ExtractionResult:
        ...
        try:
            ...
            logger.error(f"Fatal error formatting Groq result: {e}")
